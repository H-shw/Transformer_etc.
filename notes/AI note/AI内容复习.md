...
AI内容复习数学知识凸函数 拉格朗日函数 KTT最小二乘法(尤其是矩阵)基本概念与运用有监督学习和无监督学习Kmeans神经网络中的无监督方法：AutoEcoderPCA奇异值分解GAN分类问题 与 回归问题强化学习生成模型 与 判别模型过拟合过拟合的原因有哪些正则化方法ROC 曲线的原理AUC精度，召回率，F1 值交叉验证的原理过拟合 是欠拟合没有免费午餐定理奥卡姆剃刀原理神经网络训练时的目标函数是否为凸函数凸函数与凸优化混淆矩阵的概念贝叶斯 先验概率 后验概率贝叶斯 拉普拉斯光滑决策树决策树 预剪枝 后剪枝决策树 属性缺失k 的取值对 k 近邻算法的影响。KNN用于各种距离LMNN 算法数据降维解释 PCA 降维算法的流程向量重构的流程为：神经网络 激活函数sigmoid神经网络参数的初始值如何设定？ 梯度消失动量项的原理机器学习基本模型感知机简单介绍Loss普通形式感知机学习算法的对偶形式：K邻近普通数据归一化：改进策略1：高斯函数对距离加权KD树线性回归(linear regression)回归与分类的差别方法Loss 更新参数方法Logistic回归决策树ID3C4.5CART剪枝支持向量机线性可分支持向量机线性支持向量机核函数和非线性支持向量机核函数线性核高斯核函数SMO集成学习--概述Baggingbooting
...
它基于反向传播算法与最优化方法(如梯度下降法)，利用输入数据X本身作为监督,来指导神经网络尝试学习一个映射关系, 从而得到一个重构输出xR。在时间序列异常检测场景下,异常对于正常来说是少数,所以我们认为，如果使用自编码器重构出来的输出xR跟原始输入的差异超出一定阈值(threshold) 的 话，原始时间序列即存在了异常。

通过算法模型包含两个主要的部分: **Encoder** (编码器)和 **Decoder** (解码器)。

编码器的作用是把高维输入 X 编码成低维的隐变量 h 从而强迫神经网络学习最有信息量的特征;

解码器的作用是把隐藏层的隐变量h还原到初始维度,最好的状态就是解码器的输出能够完美地或者近似恢复出原来的输入,即 xR≈x .

![image-20220603091957051](file://C:/Users/14163/AppData/Roaming/Typora/typora-user-images/image-20220603091957051.png?lastModify=1654442840)

(1)从输入层->隐藏层的原始数据X的编码过程: h=gθ1(x)=σ(W1x+b1)

(2)从隐藏层->输出层的解码过程:

x^=gθ2(h)=σ(W2h+b2)

那么算法的优化目标函数就写为: MinimizeLoss=dist(X,XR) 其中dist为二者的距离度量函数,通常用 MSE (均方方差)。

MSE=1n∑i=1mwi(yi−yi^)2
...
[https://zh.wikipedia.org/wiki/%E5%A5%87%E5%BC%82%E5%80%BC%E5%88%86%E8%A7%A3](https://zh.wikipedia.org/wiki/奇异值分解)
...
- L2正则化是指权值向量w中各个元素的平方和然后再求平方根（可以看到Ridge回归的L2正则化项有平方符号），通常表示为∥w∥2
- L2防止过拟合的原因：**拟合过程中通常都倾向于让权值尽可能小**，最后构造一个所有参数都比较小的模型。因为一般认为参数值小的模型比较简单，能适应不同的数据集，也在一定程度上避免了过拟合现象。**可以设想一下对于一个线性回归方程，若参数很大，那么只要数据偏移一点点，就会对结果造成很大的影响；但如果参数足够小，数据偏移得多一点也不会对结果造成什么影响**，专业一点的说法是**抗扰动能力强**。
- θj=θj(1−αλn)−α1n∑i=1n(hθ(x(i))−y(i))xj(i)
- **其中λ就是正则化参数**。从上式可以看到，与未添加L2正则化的迭代公式相比，每一次迭代，θj 都要先乘以一个小于1的因子，从而使得θj不断减小，因此总得来看，θ是不断减小的。 最开始也提到L1正则化一定程度上也可以防止过拟合。之前做了解释，当L1的正则化系数很小时，得到的最优解会很小，可以达到和L2正则化类似的效果。
...
![image-20220603161925009](file://C:/Users/14163/AppData/Roaming/Typora/typora-user-images/image-20220603161925009.png?lastModify=1654442840)
...
对于 k 分类问题，混淆矩阵为 k * k 的矩阵，它的元素 cij 表示第i 类样本被分类器判定 为第 j 类的数量。
...
在类条件概率的计算公式中，如果 , Nxi=v,y=c 为 0，即特征分量的某个取值在某一类在训 练样本中一次都不出现，则会导致如果预测样本的特征分量取到这个值时**整个分类判别函数 的值为 0**。
...
Gini 不纯度。样本集的 Gini 不纯度定义：G(D)=1−∑ipi2

C4.5

GR=GIV

其中 IV=−∑i=1m|Di||D|ln|Di||D|
...
![image-20220603200815494](file://C:/Users/14163/AppData/Roaming/Typora/typora-user-images/image-20220603200815494.png?lastModify=1654442840)
...
Xavier等人发现，为了使各层的激活值呈现出具有相同广度的分布，推导了合适的权重尺度。推导出的结论是，如果前一层的节点数为n，则初始值使用标准差为 1n 的分布。

**He初始值（ReLU的权重初始值）**

当前一层的节点数为n时，He初始值使用标准差为 2n 的高斯分布。

##### 梯度消失

什么是梯度消失问题，为什么会出现梯度消失问题？ 在用反向传播算法计算误差项时每一层都要乘以本层激活函数的导数。

δl=(Wl+1)Tδ(l+1)⊙f′(u(l))
...
它是上一时刻的动量项与本次梯度值的加权平均值，其中 α 是学习率， μ 是动量项 数。如果按照时间 t 进行展开，则第 t 次迭代时使用了从 1 到t 次迭代时的所有梯度值，且老 的梯度值按照 μt 的系数指数级衰减。动量项是为了加快梯度下降法的收敛，它使用历史信息对当前梯度值进行修正，以抵消在病态条件问题上的来回震荡。
...
<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220605150927566.png" alt="image-20220605150927566" style="zoom:67%;" />
...
<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220605155627740.png" alt="image-20220605155627740" style="zoom:67%;" />
...
<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220605155736804.png" alt="image-20220605155736804" style="zoom:67%;" />

上述的构建过程结合下图可以看出，构建一个k-d tree即是将一个二维平面逐步划分的过程。

<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220605155748290.png" alt="image-20220605155748290" style="zoom:67%;" />
...
<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220604183423117.png" alt="image-20220604183423117" style="zoom:50%;" />

<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220604183441494.png" alt="image-20220604183441494" style="zoom: 50%;" />

两者差别

![image-20220604183515308](C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220604183515308.png)
...
![image-20220604183914658](C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220604183914658.png)
...
<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220605160828340.png" alt="image-20220605160828340" style="zoom:67%;" />
...
<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220605163554862.png" alt="image-20220605163554862" style="zoom:67%;" />
...
<img src="C:\Users\14163\AppData\Roaming\Typora\typora-user-images\image-20220605163703761.png" alt="image-20220605163703761" style="zoom:67%;" />
...
给定样本集DD和连续属性 A ，假定A 在D 上出现了 n 个不同的取值，将这些值从小到大进行排序，记为$\{a^1,a^2,...,a^n\}$。基于划分点t 可将D 分为子集$D_t^-$ 和$D_t^+$ ，其中 $D_t^-$ 包含那些在属性aa上取值不大于t 的样本，而$D_t^+$ 则包含那些在属性aa上取值大于tt的样本。显然，对相邻的属性取值$a^i$
